...
#import "Macros.typ": stInitial, stOpen, stClosed, stFinal
#import "Macros.typ": txCommit, txCollectCom, txDecrement
...
The initiator then establishes the head by submitting an _initial_
transaction to the mainchain that contains the Hydra protocol parameters and
mints special _participation tokens (PT)_ identifying the head members. The
_initial_ transaction also initializes a state machine (see
@figure:head-protocol-states) that manages the ``transfer'' of UTxOs into
the head and back. The state machine comprises the four states: $stInitial$,
$stOpen$, $stClosed$, and $stFinal$. A _state thread token (ST)_ minted
in _initial_ marks the head output and ensures contract continuity @eutxo.

#include("Figures/head-protocol-states.typ")

Once the initial transaction appears on the mainchain, establishing the initial
state $stInitial$, each head member can attach a $txCommit$ transaction, which
...
The commit transactions are subsequently collected by the $txCollectCom$ transaction
...
fail to post a $txCommit$ transaction, the head can be aborted by going directly
...
set of UTxOs that is identical to the UTxOs locked on-chain via the $txCommit$
and $txCollectCom$ transactions.
...
UTxO set, while snapshots thereafter $\U_1,\U_2,dots$ are created with
...
chain via a $txDecrement)$ @sec:decrement-tx transaction --- the overall process is also called
``decommit``.
/*

#text(red)[The inverse of locking more UTxO on the mainchain and making it available
	within the head is called (incremental) ``commit``. First, anyone may create a
	deposit of one or more UTxO using a $sans("txDeposit")$ @sec:deposit-tx
	...
	of a snapshot. With that agreement, an $sans("txIncrement")$ @sec:increment-tx
	...
	$sans("txRecover")$ @sec:recover-tx transaction allows anyone to unlock the
	original UTxO.])
*/
=== Closing the head

...
\noindent \red{Besides the main state transitions of the head protocol, there is
...
\noindent \red{The deposit protocol defines one validator script:
  \begin{itemize}
...
/*
The $txInit$ transaction creates a head instance and establishes the initial
...
\begin{figure}
  ...
  \caption{\mtxInit{} transaction spending a seed UTxO, and producing the head
	output in state $sans("stInitial")$ and initial outputs for each participant.}\label{fig:initTx}
\end{figure}

\noindent Two kinds of tokens are minted:
- A single \emph{State Thread (ST)} token marking the head output. This
...
- One \emph{Participation Token (PT)} per participant
...
- has $|\hydraKeys|$ initial outputs $o_{\mathsf{initial}_{i}}$ with datum $\datumInitial{} = \cid$,
- has one head output
  $o_{\mathsf{head}}$, which captures
  the initial state of the protocol in the datum
  \[
	\datumHead = (sans("stInitial"),\cid',\seed',\hydraKeys,\cPer)
  \]
  where
  - sans("stInitial")$ is a state identifier,
  - $\cid'$ is the unique currency id of this instance,
  - $\seed'$ is the output reference parameter of $\muHead$,
  - $\hydraKeys$ are the off-chain multi-signature keys from the setup
  	phase,
  - $\cPer$ is the contestation period.
...
- When evaluated with the $\mathsf{Mint}$ redeemer,
  ...
  - All entries of $\txMint$ are of this policy and of single quantity $\forall \{c \mapsto \cdot \mapsto q\} \in \txMint : c = \cid \land q = 1$
  - Right number of tokens are minted $|\txMint| = |\hydraKeys| + 1$
	% TODO: |\txMint| may not be clear to the reader, maybe combine with item above, but be more explicit.
  - State token is sent to the head validator $\st \in \valHead$ % TODO: fact that it is goverend by nuHead is a bit implicit here.
  - \textcolor{blue}{The correct number of initial outputs are present $|(\cdot, \nuInitial, \cdot) \in \txOutputs| = |\hydraKeys|$}
	% XXX: this is implied by the ledger, so can be removed
  - All participation tokens are sent to the initial validator as an initial output $\forall i \in [1 \dots |\hydraKeys|] : \{\cid \mapsto \cdot \mapsto 1\} \in \valInitial{i}$
  - The $\datum_{\mathsf{head}}$ contains own currency id $\cid = \cid'$ and the right seed reference $\seed = \seed'$
  - All initial outputs have a $\cid$ as their datum: $\forall i \in [1 \dots |\hydraKeys|] : \cid = \datumInitial{i}$
- When evaluated with the $\mathsf{Burn}$ redeemer,\todo{move to abort/fanout?}
  - All tokens for this policy in $\txMint$ need to be of negative quantity
...
\begin{menumerate}
  \item State is advanced from $\datumHead \sim sans("stInitial")$ to terminal state
  $sans("stFinal")$:
  \[
	(sans("stInitial"),\cid,\seed,\hydraKeys,\cPer) \xrightarrow[m]{\stAbort} sans("stFinal").
  \]
...
\begin{figure}
  ...
  \caption{\mtxAbort{} transaction spending the $sans("stInitial")$ state head
...
\begin{menumerate}
  \item State is advanced from $\datumHead \sim sans("stInitial")$ to
  $\datumHead' \sim sans("stOpen")$, parameters $\cid,\hydraKeys,\cPer$ stay
  unchanged and the new state is governed again by $\nuHead$
  \[
	(sans("stInitial"),\cid,\seed,\hydraKeys,\cPer) \xrightarrow{\stCollect} (sans("stOpen"),\cid,\hydraKeys,\cPer,v,\eta)
  \]
...
\begin{figure}
  ...
  \caption{\mtxCCom{} transaction spending the head output in $sans("stInitial")$
	state and collecting from multiple commit outputs into a single
	$sans("stOpen")$ head output.}\label{fig:collectComTx}
\end{figure}
*/
...
\begin{menumerate}
  ...
  unchanged and the new state is governed again by $\nuHead$
  \[
	(sans("stOpen"),\cid,\hydraKeys,\cPer,v,\eta) \xrightarrow[\xi, s, m]{\mathsf{decrement}} (sans("stOpen"),\cid,\hydraKeys,\cPer,v',\eta')
  \]
  \item New version $v'$ is incremented correctly
  \[
	v' = v + 1
...
\begin{figure}
  ...
  \caption{\mtxClose{} transaction spending the $sans("stOpen")$ head output and producing a $sans("stClosed")$ head output.}\label{fig:closeTx}
\end{figure}
...
\begin{menumerate}
  \item State is advanced from $\datumHead \sim sans("stOpen")$ to
  $\datumHead' \sim sans("stClosed")$, parameters $\cid,\hydraKeys,\cPer$
  stay unchanged and the new state is governed again by $\nuHead$
  \[
	(sans("stOpen"),\cid,\hydraKeys,\cPer,v,\eta) \xrightarrow[\mathsf{CloseType}]{\stClose} (sans("stClosed"),\cid,\hydraKeys,\cPer,v',s',\eta', \eta_\Delta', \contesters,\Tfinal)
  \]
  ...
  \item Based on the redeemer $\mathsf{CloseType} = \mathsf{Initial} \red{\cup (\mathsf{UnusedInc}, \xi, \eta_\alpha)} \cup (\mathsf{UnusedDec}, \xi) \red{\cup (\mathsf{UsedInc}, \xi)}  \cup (\mathsf{UsedDec}, \xi, \eta_\omega) $, where $\xi$ is a multi-signature of the closing snapshot \red{and $\eta_\alpha$} and $\eta_\omega$ are the digests of the UTxO to \red{increment or} decrement respectively, \red{five} cases are distinguished:
  \begin{menumerate}
...
party to prove the currently $sans("stClosed")$ state is not the latest one. This
transaction has
\begin{itemize}
  \item one input spending from $\nuHead$ holding the $\st$ with $\datumHead$,
  \item one output paying to $\nuHead$ with value $\valHead'$ and
...
\begin{figure}
  ...
  \caption{\mtxContest{} transaction spending the $sans("stClosed")$ head output and
	producing a different $sans("stClosed")$ head output.}\label{fig:contestTx}
\end{figure}
...
\begin{menumerate}
  ...
  stay unchanged and the new state is governed again by $\nuHead$
  \[
	(sans("stClosed"),\cid,\hydraKeys,\cPer,v,s,\eta,\eta_{\Delta},\contesters,\Tfinal) \xrightarrow[\mathsf{ContestType}]{\stContest} (sans("stClosed"),\cid,\hydraKeys,\cPer,v',s',\eta',\eta_{\Delta}',\contesters',\Tfinal')
  \]
...
\begin{figure}
  ...
  \caption{\mtxFanout{} transaction spending the $sans("stClosed")$ head output and
...
\begin{menumerate}
  \item State is advanced from $\datumHead \sim sans("stClosed")$ to terminal state
  $sans("stFinal")$: % XXX: What does this actually mean?
  \[
	(sans("stClosed"),\cid,\hydraKeys,\cPer,v, s,\eta,\eta_{\Delta},\contesters,\Tfinal) \xrightarrow[m,n]{\stFanout} sans("stFinal")
  \]

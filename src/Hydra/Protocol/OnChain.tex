...
\begin{itemize}
	...
	\item has one head output
	      ...
	      \[
		      \datumHead = (\stOpen,\cid',\seed',\hydraKeys,\Tcontest,v,\eta)
	      \]
	      where
	      \begin{mitemize}
...
\begin{menumerate}
	...
	\]
	and \red{$A$ is the BLS accumulator commitment of the new snapshot (see Section~\ref{sec:bls-accumulator}):
	\[
		A = \accUTxO(U')
	\]}
...
\begin{menumerate}
	...
	\]
	and \red{$A$ is the BLS accumulator commitment of the new snapshot UTxO together with the decommitted outputs (see Section~\ref{sec:bls-accumulator}):
	\[
		A = \accUTxO(U' \cup U_\omega)
	\]}
...
\begin{menumerate}
	...
	stay unchanged and the new state is governed again by $\nuHead$
	\[
		(\stOpen,\cid,\hydraKeys,\Tcontest,v,\eta) \xrightarrow[\mathsf{CloseType}]{\stClose} (\stClosed,\cid,\hydraKeys,\Tcontest,v,s',\eta_{\mathsf{snap}},\red{\eta_\alpha,\eta_\omega,A},\contesters,\tfinal)
	\]
	where the closed datum stores the snapshot UTxO hash $\eta_{\mathsf{snap}}$, and \red{three new fields: $\eta_\alpha$ (hash of committed UTxO, or $\bot$), $\eta_\omega$ (hash of decommitted UTxO, or $\bot$), and $A = \accUTxO(\cdot)$ the BLS accumulator commitment used for partial fanout verification.} These fields are set by the closer as described in each case below.
	\item Last known open state version is recorded in closed state
	\[
		v' = v
	\]

	\item Based on the redeemer $\mathsf{CloseType} = \mathsf{Initial} \cup (\mathsf{Any}, \xi, A) \cup (\mathsf{UnusedInc}, \xi, \eta_\alpha, A) \cup (\mathsf{UnusedDec}, \xi, A) \cup (\mathsf{UsedInc}, \xi, \eta_\alpha, A) \cup (\mathsf{UsedDec}, \xi, \eta_\omega, A)$, where $\xi$ is a multi-signature of the closing snapshot, $\eta_\alpha$ and $\eta_\omega$ are hashes of the committed or decommitted UTxO, and \red{$A$ is the BLS accumulator commitment}, six cases are distinguished.
	In all cases except $\mathsf{Initial}$, \red{the accumulator $A$ in the redeemer must equal the accumulator commitment $A' = \accUTxO(\cdot)$ stored in the closed datum:
	\[
		A = A'
	\]}
	\begin{menumerate}
...
\begin{menumerate}
	...
	In all cases, \red{the accumulator $A$ in the redeemer must equal the accumulator commitment stored in the new closed datum.}
...
\noindent The state-machine validator $\nuHead$ is spent with
$\redeemerHead = (\stPartialFanout, m, \mathit{crsRef})$, where $m$ is the
number of outputs to distribute and $\mathit{crsRef}$ references the common
reference string, and checks:
\begin{menumerate}
  \item State is advanced to $\datumHead' \sim \stFanoutProgress$ with head
        ...
        \[
          ...
          \xrightarrow[m,\mathit{crsRef}]{\stPartialFanout}
...
\noindent The state-machine validator $\nuHead$ is spent with
$\redeemerHead = (\mathsf{fanout}, m, n, n', \red{\pi, \mathit{crsRef}})$, where $m$ is
the number of main UTxO outputs, \red{$n$ and $n'$ are the number of committed and decommitted UTxO outputs respectively, $\pi$ is a BLS membership proof and
$\mathit{crsRef}$ references the common reference string}, and checks:
\begin{menumerate}
	...
	\[
		...
		\xrightarrow[m,n,n',\red{\pi,\mathit{crsRef}}]{\stFanout} \stFinal
	\]
	...
	\]
	\item \red{All distributed outputs are verified against the BLS accumulator commitment:
	\[
		\membershipVfy(A,\; \{o_1, \dots, o_{m+n+n'}\},\; \pi,\; \mathit{crs}) = \true
	\]}
...
\noindent The state-machine validator $\nuHead$ is spent with
$\redeemerHead = (\mathsf{finalPartialFanout}, m, \pi, \mathit{crsRef})$, where
$m$ is the number of remaining outputs, $\pi$ is a membership proof for those
outputs, and $\mathit{crsRef}$ references the common reference string, and checks:
\begin{menumerate}
	\item State is advanced to the terminal state $\stFinal$ from
	$\stFanoutProgress$:
	\[
		(\stFanoutProgress,\cid,\hydraKeys,\tfinal,A)
		\xrightarrow[m,\pi,\mathit{crsRef}]{\mathsf{finalPartialFanout}} \stFinal
	\]

...
\noindent \red{Besides the main state transitions of the head protocol,
	there is the related ``deposit protocol'' with two transactions in support of
	\mtxIncrement{}: \mtxDeposit{} and \mtxRecover{}.} \\
...
\noindent \red{The deposit protocol defines one minting policy and one validator
	script:
	\begin{itemize}
		\item $\muDeposit$ governs minting of a deposit thread token and ensures correct
		      recording of UTxO in a \mtxDeposit{} transaction.
		\item $\nuDeposit$ controls that deposited UTxO is claimed correctly into a
		      head via \mtxIncrement{} or recovered after the deadline has passed in a
		      \mtxRecover{} transaction.
	\end{itemize}
}
...
\subsection{\red{Deposit Transaction}}\label{sec:deposit-tx}

\noindent The \mtxDeposit{} transaction initiates a (incremental) commit by
locking funds on the mainchain for later collection by the head protocol. The
validators in this transaction ensures that anything deposited is correctly
recorded in the datum and allow for either collection or recovery of the
deposit. The transaction is shown in Figure~\ref{fig:depositTx} and it

\begin{itemize}
	\item has one or more inputs with reference $\txOutRef_{\mathsf{deposited}_{j}}$
	      spending output $o_{\mathsf{deposited}_{j}}$ with
	      $\val_{\mathsf{deposited}_{j}}$
	\item mints a \emph{Deposit Token (DT)} identifying this deposit in token name $\tau_{\mathsf{deposit}}$
	      \[
		      \dt = \{\did \mapsto \tau_{\mathsf{deposit}} \mapsto 1\}
	      \]
	\item has one output paying to $\nuDeposit$ with value $\valDeposit$ and datum
	      \[
		      \datumDeposit = (\cid, \did, t_{\mathsf{recover}}, C)
	      \]
	      where
	      \begin{mitemize}
		      \item $\cid$ is the currency id of the target head protocol instance (see~\ref{sec:init-tx}),
		      \item $\did$ is the currency id of the $\dt$,
		      \item $t_{\mathsf{recover}}$ is a deadline after which the deposit can be recovered, and
		      \item $C$ is the list of deposited UTxO (similar to commits in~\ref{sec:commit-tx}).
	      \end{mitemize}
\end{itemize}

\noindent The $\muDeposit$ minting policy is the only script that verifies
deposit transactions and can be redeemed with either a $\mathsf{Mint}$ or
$\mathsf{Burn}$ redeemer:\todo{ALL of this could be done off-chain, no need for token + deposit checks?}
\begin{itemize}
	\item When $\redeemerDeposit = (\mathsf{Mint}, \overline{\txOutRef}_{\mathsf{deposited}})$,
	      \begin{menumerate}
		      \item All deposited value is in the output\footnote{The$\supseteq$ is
			      important for real world situations where the values might not be
			      exactly equal due to ledger constraints (i.e. to ensure a minimum
			      value on outputs).}
		      \[
			      \valDeposit \supseteq \bigcup_{j=1}^{m} \val_{\mathsf{deposited}_{j}}
		      \]
		      \item Deposited outputs are recorded in the output datum
		      \[
			      C = \forall j \in \{1 \dots m\} : [(\txOutRef_{\mathsf{deposited}_{j}},\bytes(o_{\mathsf{deposited}_{j}}))]
		      \]
	      \end{menumerate}
	\item When $\redeemerDeposit = \mathsf{Burn}$,\todo{move to Claim/Recover?}
	      \begin{menumerate}
		      \item All tokens for this policy in $\txMint$ need to be of negative quantity
		      \[
			      \forall \{\cid \mapsto \cdot \mapsto q\} \in \txMint : q < 0
...
\subsection{\red{Recover Transaction}}\label{sec:recover-tx}
...
\begin{itemize}
	...
	\item one output paying to $\nuHead$ with value $\valHead'$ and
	      datum $\datumHead'$,
\end{itemize}
\todo{continue with deposit draft}
...
\begin{menumerate}
	...
	\[
		v' = v + 1
	\]
	\item $\xi$ is a valid multi-signature of the new snapshot state (currency id $\cid$, the current state version $v$, the new state $\eta'$ \red{, UTxO to deposit $\eta_\alpha$} and UTxO to withdraw $\eta_\omega$)
	\[

...
on-chain: \mtxInit{}~\ref{sec:init-tx}, \mtxIncrement{}~\ref{sec:increment-tx}, \mtxDecrement{}~\ref{sec:decrement-tx}, \mtxClose{}~\ref{sec:close-tx}, \mtxContest{}~\ref{sec:contest-tx}, \mtxFanout{}~\ref{sec:fanout-tx}, \textcolor{red}{\mtxPartialFanout{}~\ref{sec:partial-fanout-tx}, and \mtxFinalPartialFanout{}~\ref{sec:final-partial-fanout-tx}}. \\

\noindent \textcolor{red}{The protocol uses KZG accumulators (see Section~\ref{sec:accumulators}) to enable partial fanout when UTxO sets exceed transaction size limits. When all UTxOs fit in a single transaction, \mtxFanout{} distributes them all at once. When UTxO sets are too large, \mtxPartialFanout{} distributes subsets across multiple transactions using membership witnesses, transitioning through an intermediate $\stFanoutProgress$ state, until \mtxFinalPartialFanout{} completes the distribution.} \\
...
\begin{itemize}
	...
	\item $\nuHead$ represents the main protocol state machine logic and ensures
	      ...
	      $\mtxClose{}$, $\mtxContest{}$, $\mtxFanout{}$, \textcolor{red}{$\mtxPartialFanout{}$ and $\mtxFinalPartialFanout{}$}.
\end{itemize}
...
\begin{itemize}
	...
	\item has one head output
	      ...
	      \[
		      \datumHead = (\stOpen,\cid',\seed',\hydraKeys,\Tcontest,v,\eta,\mathsf{ada\_o})
	      \]
	      where
	      \begin{mitemize}
...
\begin{menumerate}
	...
	\[
		(\stOpen,\cid,\hydraKeysAgg,\nop,\Tcontest,v,\eta,\mathsf{ada\_o}) \xrightarrow[\xi, s, \txOutRef_{\mathsf{increment}}]{\mathsf{increment}} (\stOpen,\cid,\hydraKeysAgg,\nop,\Tcontest,v',\eta',\mathsf{ada\_o})
	\]
	...
	\]
	\item \textcolor{red}{The ADA overhead $\mathsf{ada\_o}$ is preserved across the state transition:
	\[
		\mathsf{ada\_o}' = \mathsf{ada\_o}
	\]}
\end{menumerate}
...
\begin{menumerate}
	...
	\[
		(\stOpen,\cid,\hydraKeys,\Tcontest,v,\eta,\mathsf{ada\_o}) \xrightarrow[\xi, s, m]{\mathsf{decrement}} (\stOpen,\cid,\hydraKeys,\Tcontest,v',\eta',\mathsf{ada\_o})
	\]
	\item New version $v'$ is incremented correctly
	\[
		v' = v + 1
...
\begin{menumerate}
	...
	stay unchanged and the new state is governed again by $\nuHead$
	\[
		\textcolor{red}{(\stOpen,\cid,\hydraKeys,\Tcontest,v,\eta,\mathsf{ada\_o}) \xrightarrow[\mathsf{CloseType}]{\stClose} (\stClosed,\cid,\hydraKeys,\Tcontest,v',s',\eta', \contesters,\tfinal,\mathsf{ada\_o})}
	\]
	\textcolor{red}{The closed state carries a single unified accumulator $\eta'$ that combines the snapshotted UTxO set with any pending increment or decrement UTxOs using $\accCombine$.}
	\item Last known open state version is recorded in closed state
	\[
		v' = v
	\]

	\item \textcolor{red}{Based on the redeemer $\mathsf{CloseType} = \mathsf{Initial} \cup (\mathsf{Any}, \xi, {\eta'}^{\#}) \cup (\mathsf{Unused}, \xi, {\eta'}^{\#}) \cup (\mathsf{Used}, \xi, {\eta'}^{\#})$, where $\xi$ is a multi-signature of the closing snapshot and ${\eta'}^{\#}$ is the hash of the unified accumulator commitment stored in the output datum, four cases are distinguished. In each case the closed state carries a single unified accumulator $\eta'$:}
	\begin{menumerate}
		...
		\]
		\[
			\textcolor{red}{\eta' = \accUTxO(\emptyset)}
		\]
		\item $\mathsf{Any}$: Closing snapshot refers to current state version $v$ with no pending increments or decrements, and $s' > 0$. \textcolor{red}{The unified accumulator is simply the snapshotted state:}
		\[
			\textcolor{red}{\eta' = \accUTxO(U')}
		\]
		\[
			\msVfy(\hydraKeys,(\cid || v || s' || {\eta'}^{\#}),\xi) = \true
		\]
		\[
			\textcolor{red}{{\eta'}^{\#} = (\eta')^{\#}}
		\]
		\item $\mathsf{Unused}$: Closing snapshot refers to current state version $v$ and a pending increment or decrement is \emph{not} applied in the snapshot. \textcolor{red}{The unified accumulator is the snapshotted state only:}
		\[
			\textcolor{red}{\eta' = \accUTxO(U')}
		\]
		\[
		...
		\[
			\textcolor{red}{{\eta'}^{\#} = (\eta')^{\#}}
		\]
		\item $\mathsf{Used}$: Closing snapshot refers to the previous state version $v - 1$ and a pending increment or decrement \emph{is} applied in the snapshot. \textcolor{red}{The unified accumulator combines the snapshotted UTxOs with the pending delta:}
		\[
			\textcolor{red}{\eta' = \accCombine(\accUTxO(U'), \eta_\Delta)}
		\]
		\[
		...
		\[
			\textcolor{red}{{\eta'}^{\#} = (\eta')^{\#}}
		\]
		\textcolor{red}{where $\eta_\Delta$ is the accumulator commitment of the pending delta UTxOs.}
	\end{menumerate}
	...
	\[
		\txMint = \varnothing
	\]
	\item \textcolor{red}{The ADA overhead $\mathsf{ada\_o}$ is propagated unchanged from the open datum to the closed datum:
	\[
		\mathsf{ada\_o}' = \mathsf{ada\_o}
	\]
	where $\mathsf{ada\_o}$ is the ADA in the head UTxO not belonging to any L2 UTxO (minimum-UTxO overhead), set at initialisation time and unchanged for the head's lifetime.}
\end{menumerate}

\begin{figure}[H]
	\centering
	\includesvg[width=\textwidth]{Hydra/Protocol/Figures/closeTx.svg}
	\caption{\mtxClose{} transaction spending the $\stOpen$ head output and producing a $\stClosed$ head output \textcolor{red}{with unified accumulator $\eta'$.}}\label{fig:closeTx}
\end{figure}
...
\begin{menumerate}
	...
	stay unchanged and the new state is governed again by $\nuHead$
	\[
		\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v,s,\eta,\contesters,\tfinal,\mathsf{ada\_o}) \xrightarrow[\mathsf{ContestType}]{\stContest} (\stClosed,\cid,\hydraKeys,\Tcontest,v',s',\eta',\contesters',\tfinal',\mathsf{ada\_o})}
	\]
	\textcolor{red}{The closed state carries a single unified accumulator $\eta'$ computed using $\accCombine$ based on the contest type.}
	...
	\[
		s' > s
	\]
	\item \textcolor{red}{Based on the redeemer $\mathsf{ContestType} = (\mathsf{Unused}, \xi, {\eta'}^{\#}) \cup (\mathsf{Used}, \xi, {\eta'}^{\#})$, where $\xi$ is a multi-signature of the contesting snapshot and ${\eta'}^{\#} = (\eta')^{\#}$ is the hash of the unified accumulator commitment stored in the output datum, two cases are distinguished:}

	\begin{menumerate}
		\item $\mathsf{Unused}$: Contesting snapshot refers to current state version $v$ (pending delta not applied in snapshot). \textcolor{red}{The unified accumulator reflects only the snapshotted UTxOs:}
		\[
			\textcolor{red}{\eta' = \accUTxO(U')}
		\]
		\[
		...
		\[
			\textcolor{red}{{\eta'}^{\#} = (\eta')^{\#}}
		\]
		\item $\mathsf{Used}$: Contesting snapshot refers to the previous state version $v - 1$ (pending delta applied in snapshot). \textcolor{red}{The unified accumulator combines the snapshotted UTxOs with the pending delta:}
		\[
			\textcolor{red}{\eta' = \accCombine(\accUTxO(U'), \eta_\Delta)}
		\]
		\[
	...
	\[
		\txMint = \varnothing
	\]
	\item \textcolor{red}{The ADA overhead $\mathsf{ada\_o}$ is preserved in the output closed datum:
	\[
		\mathsf{ada\_o}' = \mathsf{ada\_o}
	\]}
\end{menumerate}
...
\textcolor{red}{distributes all UTxOs from the head according to the unified accumulator in the closed state. A fanout transaction consists of}
\begin{itemize}
	\item one input spending from $\nuHead$ holding the $\st$, and
	\item \textcolor{red}{outputs $o_{1} \dots o_{m}$ to distribute all UTxOs.}
\end{itemize}

\noindent \textcolor{red}{The state-machine validator $\nuHead$ is spent with
...
\begin{menumerate}
	...
	\[
		\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v, s,\eta,\contesters,\tfinal) \xrightarrow[m,\pi]{\stFanout} \stFinal}
	\]
	\item \textcolor{red}{All $m$ outputs are verified as members of the unified accumulator $\eta$ using the membership witness $\pi$:}
	\[
		\textcolor{red}{\accVerify(\eta, \{o_1, \ldots, o_m\}, \pi) = \true}
	\]
	...
	\item \textcolor{red}{The head input value is fully conserved:
	\[
		\val_{\mathsf{head}}^{\mathsf{in}} = \bigoplus_{i=1}^{m} \val(o_i) \oplus \val_{\mathsf{burned}} \oplus \mathsf{ada\_o}
	\]
	where $\mathsf{ada\_o}$ is the ADA overhead carried from the closed datum, and $\val_{\mathsf{burned}}$ is the value of all burned head tokens.}
\end{menumerate}

\begin{figure}[H]
	\centering
	\includesvg[width=\textwidth]{Hydra/Protocol/Figures/fanoutTx.svg}
	\caption{\mtxFanout{} transaction spending the $\stClosed$ head output \textcolor{red}{with unified accumulator $\eta$} and
...
\textcolor{red}{When UTxO sets exceed transaction size limits, the protocol distributes UTxOs across
...
subsequent steps:}
\[
  \textcolor{red}{(\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta,\mathsf{ada\_o})}
\]
\textcolor{red}{where $\tfinal$ is the contestation deadline (carried from $\stClosed$), $\eta$ is the
current accumulator commitment, and $\mathsf{ada\_o}$ is the ADA overhead in the head UTxO not
belonging to any L2 UTxO (propagated from the closed datum).}

\textcolor{red}{An intermediate partial fanout transaction (see Figure~\ref{fig:partialFanoutTx}) consists of:}
\begin{itemize}
  \item \textcolor{red}{one input spending from $\nuHead$ in state $\stClosed$ or $\stFanoutProgress$, and}
  \item \textcolor{red}{a continuing head output at index 0 in state $\stFanoutProgress$ with updated accumulator, and}
  \item \textcolor{red}{outputs $o_{1} \dots o_{m}$ at indices $1 \dots m$ distributing a subset of UTxOs.}
\end{itemize}
...
The validator checks:
\begin{menumerate}
  \item \textcolor{red}{$m > 0$ (no zero-output batches).}
  \item \textcolor{red}{State transitions into $\stFanoutProgress$ with updated accumulator. For a $\stClosed$ input:}
  \[
	\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v,s,\eta,\contesters,\tfinal) \xrightarrow[m]{\stPartialFanout} (\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta',\mathsf{ada\_o})}
  \]
  \textcolor{red}{For a $\stFanoutProgress$ input:}
  \[
	\textcolor{red}{(\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta,\mathsf{ada\_o}) \xrightarrow[m]{\stPartialFanout} (\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta',\mathsf{ada\_o})}
  \]
  \item \textcolor{red}{The new accumulator $\eta'$ (from the output datum) is not the G1 generator — all elements have \emph{not} yet been removed (use $\stFinalPartialFanout$ for the last batch):}
  \[
    \textcolor{red}{\eta' \neq G_1}
  \]
  \item \textcolor{red}{No minting or burning $\txMint = \varnothing$.}
  \item \textcolor{red}{Transaction is posted after contestation deadline $\txValidityMin > \tfinal$.}
  \item \textcolor{red}{Parameters $\cid$, $\hydraKeys$, $\tfinal$, and $\mathsf{ada\_o}$ are preserved in the output $\stFanoutProgress$ datum.}
  \item \textcolor{red}{Value is conserved: head input value equals head output value plus all distributed outputs:}
  \[
    \textcolor{red}{\val_{\mathsf{head}}^{\mathsf{in}} = \val_{\mathsf{head}}^{\mathsf{out}} \oplus \bigoplus_{i=1}^{m} \val(o_i)}
  \]
  \item \textcolor{red}{The new accumulator $\eta'$ from the output datum correctly represents the remaining UTxOs after removing $S = \{o_1, \ldots, o_m\}$. The output datum value serves as the exclusion witness:}
  \[
	\textcolor{red}{\accVerifyExclude(\eta, S, \eta') = \true}
  \]
\end{menumerate}
...
\textcolor{red}{Once all UTxOs except the last batch have been distributed via $\stPartialFanout$ steps,
...
\begin{itemize}
  ...
  \item \textcolor{red}{outputs $o_{1} \dots o_{m}$ distributing the remaining UTxOs (no continuing head output).}
\end{itemize}
...
The validator checks:
\begin{menumerate}
  \item \textcolor{red}{$m > 0$ (prevents fund theft via a zero-output proof bypass).}
  \item \textcolor{red}{State is advanced from $\stFanoutProgress$ to terminal state $\stFinal$:}
  \[
	\textcolor{red}{(\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta,\mathsf{ada\_o}) \xrightarrow[m,\pi]{\stFinalPartialFanout} \stFinal}
  \]
  \item \textcolor{red}{All head tokens are burnt
  $|\{\cid \mapsto \cdot \mapsto -1\} \in \txMint| = n + 1$.}
  \item \textcolor{red}{Transaction is posted after contestation deadline $\txValidityMin > \tfinal$.}
  \item \textcolor{red}{The $m$ distributed outputs are verified as members of the accumulator $\eta$ using the membership witness $\pi$:}
  \[
	\textcolor{red}{\accVerify(\eta, \{o_1, \ldots, o_m\}, \pi) = \true}
  \]
  \item \textcolor{red}{Value is conserved:}
  \[
    \textcolor{red}{\val_{\mathsf{head}}^{\mathsf{in}} = \bigoplus_{i=1}^{m} \val(o_i) \oplus \val_{\mathsf{burned}} \oplus \mathsf{ada\_o}}
  \]
\end{menumerate}

...
\begin{itemize}
	...
	\item has one head output
	      ...
	      \begin{mitemize}
		      ...
		      \item $\eta = \combine(\emptyset)$ is the canonical hash of the (empty) initial UTxO set,
...
\begin{menumerate}
	...
	stay unchanged and the new state is governed again by $\nuHead$
	\[
		(\stOpen,\cid,\hydraKeys,\Tcontest,v,\eta,\red{A}) \xrightarrow[\mathsf{CloseType}]{\stClose} (\stClosed,\cid,\hydraKeys,\Tcontest,v',s',\eta', \eta_\alpha\Delta', \eta_\omega\Delta', \contesters,\tfinal)
	\]
	...
	\item Based on the redeemer $\mathsf{CloseType} = \mathsf{Initial} \cup (\mathsf{Any}, \xi) \cup (\mathsf{UnusedInc}, \xi, \eta_\alpha) \cup (\mathsf{UnusedDec}, \xi) \cup (\mathsf{UsedInc}, \xi, \eta_\alpha) \cup (\mathsf{UsedDec}, \xi, \eta_\omega)$, where $\xi$ is a multi-signature of the closing snapshot and $\eta_\alpha$ and $\eta_\omega$ are the digests of the UTxO to increment or decrement respectively, six cases are distinguished.
	...
	\begin{menumerate}
		...
		\]
		\[
			\msVfy(\hydraKeys,(\cid || v || s' || \eta' || \eta_\alpha || \eta_\omega || \red{A}),\xi) = \true
		\]
		...
		\]
		\[
			\msVfy(\hydraKeys,(\cid || v - 1 || s' || \eta' || \eta_\alpha || \eta_\omega || \red{A}),\xi) = \true
		\]
...
\begin{menumerate}
	...
	stay unchanged and the new state is governed again by $\nuHead$
	\[
		(\stClosed,\cid,\hydraKeys,\Tcontest,v,s,\eta,\eta_\alpha\Delta,\eta_\omega\Delta,\contesters,\tfinal) \xrightarrow[\mathsf{ContestType}]{\stContest} (\stClosed,\cid,\hydraKeys,\Tcontest,v',s',\eta',\eta_\alpha\Delta',\eta_\omega\Delta',\contesters',\tfinal')
	\]
	...
	\[
		s' > s
	\]
	\item Based on the redeemer $\mathsf{ContestType} = (\mathsf{Current}, \xi, A) \cup (\mathsf{UnusedInc}, \xi, \eta_\alpha, A) \cup (\mathsf{UnusedDec}, \xi, A) \cup (\mathsf{UsedInc}, \xi, A) \cup (\mathsf{UsedDec}, \xi, \eta_\omega, A)$, where $\xi$ is a multi-signature of the contesting snapshot, $\eta_\alpha$ and $\eta_\omega$ are the digests of the UTxO to increment or decrement respectively, and \red{$A$ is the BLS accumulator commitment}, five cases are distinguished:
...
\begin{menumerate}
  \item State is advanced to $\datumHead' \sim \stFanoutProgress$ with head
        ...
        \[
          (\stClosed,\cid,\hydraKeys,\Tcontest,v,s,\eta',\eta_\alpha\Delta,\eta_\omega\Delta,A,\contesters,\tfinal)
...
\begin{menumerate}
	...
	$\stFinal$:
	\[
		(\stClosed,\cid,\hydraKeys,\Tcontest,v,s,\eta',\eta_\alpha\Delta,\eta_\omega\Delta,\contesters,\tfinal)
		\xrightarrow[m,n,n']{\stFanout} \stFinal
	\]
	\item The first $m$ outputs are distributing funds according to $\eta'$. That is,
	the outputs exactly correspond to the UTxO canonically combined $U^{\#}$:
	\[
		\eta' = U^{\#} = \hash(\bigoplus_{j=1}^{m} \bytes(o_{j}))
	\]
	where $\combine$ produces a canonical hash from a UTxO set by sorting outputs
	by their output references, concatenating their serialised representations,
	and hashing the result\footnote{Sorting is required to ensure a canonical
		representation which can be reproduced from the UTxO set.}.
	...
	combined $U^{\#}_{\alpha\Delta}$:
	\[
		\eta_{\alpha\Delta} = U^{\#}_{\alpha\Delta} = \hash(\bigoplus_{j=m+1}^{m+n} \bytes(o_{j}))
	\]
	...
	combined $U^{\#}_{\omega\Delta}$:
	\[
		\eta_{\omega\Delta} = U^{\#}_{\omega\Delta} = \hash(\bigoplus_{j=m+n+1}^{m+n+n'} \bytes(o_{j}))
	\]
...
$m$ is the number of remaining outputs and $\pi$ is a BLS12-381 G1 element,
and checks:
\begin{menumerate}

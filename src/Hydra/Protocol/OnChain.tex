...
\mtxFanout{} verifies the entire UTxO set against the accumulator in a single check, while
\mtxPartialFanout{} and \mtxFinalPartialFanout{} use exclusion proofs to verify and distribute
subsets of UTxOs across multiple transactions.} \\
...
\begin{menumerate}
	...
	\]
	where the closed datum stores the snapshot UTxO hash $\eta_{\mathsf{snap}}$, \red{$\eta_\alpha$ (hash of committed UTxO, or $\bot$), $\eta_\omega$ (hash of decommitted UTxO, or $\bot$), and $A$ the BLS accumulator commitment carried forward from the input open datum.} The fields $\eta_\alpha$ and $\eta_\omega$ are set by the closer as described in each case below.
	...
	\red{In all cases except $\mathsf{Initial}$, the BLS accumulator commitment $A$ is taken from the input open datum and stored unchanged in the closed output datum:
	\[
		A_{\mathsf{closed}} = A_{\mathsf{open}}
	\]}
	\begin{menumerate}
...
$m$ is the number of remaining outputs and $\pi$ is a membership proof for those
outputs, and checks:
\begin{menumerate}
	...
	\]
	\item The $m$ remaining outputs are verified against the accumulator commitment $A$ from the
	$\stFanoutProgress$ datum:
	\[
		\membershipVfy(A,\; \{o_1, \dots, o_m\},\; \pi,\; \mathit{crs}) = \true
	\]
	\item Transaction is posted after contestation deadline $\txValidityMin > \tfinal$.
	\item All tokens are burnt
	$|\{\cid \mapsto \cdot \mapsto -1\} \in \txMint| = |\hydraKeys| + 1$.

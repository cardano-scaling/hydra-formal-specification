...
according to the latest state. A fanout transaction consists of
\begin{itemize}
  \item one input spending from $\nuHead$ holding the $\st$, and
  \item outputs $o_{1} \dots o_{m+n+n'}$ to distribute all UTxOs.
\end{itemize}
...
\noindent The state-machine validator $\nuHead$ is spent with
$\redeemerHead = (\mathsf{fanout}, m, n, n')$, where $m$, $n$ and $n'$ are outputs
...
\begin{menumerate}
  ...
  \[
	\eta = \textcolor{red}{\accUTxO(\{o_1, \ldots, o_m\})}
  \]
  \item The following $n$ outputs are distributing funds according to
  $\eta_\alpha\Delta$. That is, the outputs exactly correspond to the UTxO set
  $U_{\alpha\Delta}$ committed in the accumulator:
  \[
	\eta_{\alpha\Delta} = \textcolor{red}{\accUTxO(\{o_{m+1}, \ldots, o_{m+n}\})}
  \]
  \item The next $n'$ outputs are distributing funds according to
  $\eta_\omega\Delta$. That is, the outputs exactly correspond to the UTxO set
  $U_{\omega\Delta}$ committed in the accumulator:
  \[
	\eta_{\omega\Delta} = \textcolor{red}{\accUTxO(\{o_{m+n+1}, \ldots, o_{m+n+n'}\})}
  \]
...
\begin{itemize}
  ...
  \item \textcolor{red}{outputs $o_{1} \dots o_{m+n+n'}$ to distribute a subset of UTxOs, and}
...
$\redeemerHead = (\mathsf{partialFanout}, m, n, n', \pi_{\eta}, \pi_{\alpha\Delta}, \pi_{\omega\Delta}, \eta', \eta'_{\alpha\Delta}, \eta'_{\omega\Delta})$, where:}
\begin{itemize}
  \item \textcolor{red}{$m$, $n$ and $n'$ are outputs to distribute from the $\stClosed$ state}
  \item \textcolor{red}{$\pi_{\eta}$, $\pi_{\alpha\Delta}$, $\pi_{\omega\Delta}$ are exclusion proofs for the distributed UTxO sets}
  \item \textcolor{red}{$\eta'$, $\eta'_{\alpha\Delta}$, $\eta'_{\omega\Delta}$ are the updated accumulator commitments for remaining UTxOs}
\end{itemize}
\textcolor{red}{The validator checks:}
\begin{menumerate}
  \item \textcolor{red}{State remains in $\stClosed$ with updated accumulators for remaining UTxOs:}
  \[
	\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v, s,\eta,\eta_\alpha\Delta,\eta_\omega\Delta,\contesters,\tfinal) \xrightarrow[m,n,n',\pi_{\eta},\pi_{\alpha\Delta},\pi_{\omega\Delta}]{\stPartialFanout} (\stClosed',\cid,\hydraKeys,\Tcontest,v, s,\eta',\eta'_{\alpha\Delta},\eta'_{\omega\Delta},\contesters,\tfinal)}
  \]
  \item \textcolor{red}{The first $m$ outputs are distributing a subset of funds according to $\eta$. That is,
  the exclusion proof $\pi_{\eta}$ verifies that the distributed outputs $S_{\eta} = \{o_1, \ldots, o_m\}$ 
  ...
  $\eta_\alpha\Delta$. That is, the exclusion proof $\pi_{\alpha\Delta}$ verifies that the distributed 
  outputs $S_{\alpha\Delta} = \{o_{m+1}, \ldots, o_{m+n}\}$ can be removed from the accumulator $\eta_{\alpha\Delta}$:}
  \[
  ...
  $\eta_\omega\Delta$. That is, the exclusion proof $\pi_{\omega\Delta}$ verifies that the distributed 
  outputs $S_{\omega\Delta} = \{o_{m+n+1}, \ldots, o_{m+n+n'}\}$ can be removed from the accumulator $\eta_{\omega\Delta}$:}
  \[
	\textcolor{red}{\accVerifyPartial(\eta_{\omega\Delta}, S_{\omega\Delta}, \pi_{\omega\Delta}) = \true}
  \]
  \item \textcolor{red}{The outgoing $\stClosed'$ datum contains the updated accumulator commitments 
  $\eta'$, $\eta'_{\alpha\Delta}$, $\eta'_{\omega\Delta}$ that correctly represent the remaining UTxOs 
  after removing the distributed sets from each respective accumulator.}

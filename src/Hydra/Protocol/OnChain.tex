...
\begin{itemize}
	...
	\item has one head output
	      ...
	      \[
		      \datumHead = (\stOpen,\cid',\seed',\hydraKeys,\Tcontest,v,\eta,\mathsf{ovhd})
	      \]
	      where
	      \begin{mitemize}
...
\begin{menumerate}
	...
	\[
		(\stOpen,\cid,\hydraKeysAgg,\nop,\Tcontest,v,\eta,\mathsf{ovhd}) \xrightarrow[\xi, s, \txOutRef_{\mathsf{increment}}]{\mathsf{increment}} (\stOpen,\cid,\hydraKeysAgg,\nop,\Tcontest,v',\eta',\mathsf{ovhd})
	\]
	...
	\]
	\item \textcolor{red}{The ADA overhead $\mathsf{ovhd}$ is preserved across the state transition:
	\[
		\mathsf{ovhd}' = \mathsf{ovhd}
	\]}
\end{menumerate}
...
\begin{menumerate}
	...
	\[
		(\stOpen,\cid,\hydraKeys,\Tcontest,v,\eta,\mathsf{ovhd}) \xrightarrow[\xi, s, m]{\mathsf{decrement}} (\stOpen,\cid,\hydraKeys,\Tcontest,v',\eta',\mathsf{ovhd})
	\]
	\item New version $v'$ is incremented correctly
	\[
		v' = v + 1
...
\begin{menumerate}
	...
	stay unchanged and the new state is governed again by $\nuHead$
	\[
		\textcolor{red}{(\stOpen,\cid,\hydraKeys,\Tcontest,v,\eta,\mathsf{ovhd}) \xrightarrow[\mathsf{CloseType}]{\stClose} (\stClosed,\cid,\hydraKeys,\Tcontest,v',s',\eta', \contesters,\tfinal,\mathsf{ovhd})}
	\]
	...
	\item \textcolor{red}{Based on the redeemer $\mathsf{CloseType} = \mathsf{Initial} \cup (\mathsf{Any}, \xi, {\eta'}^{\#}) \cup (\mathsf{Unused}, \xi, {\eta'}^{\#}) \cup (\mathsf{Used}, \xi, {\eta'}^{\#})$, where $\xi$ is a multi-signature of the closing snapshot and ${\eta'}^{\#} = (\eta')^{\#}$ is the hash of the unified accumulator commitment stored in the output datum, four cases are distinguished. In each case the closed state carries a single unified accumulator $\eta'$:}
	\begin{menumerate}
	...
	\[
		\txMint = \varnothing
	\]
	\item \textcolor{red}{The ADA overhead $\mathsf{ovhd}$ is propagated unchanged from the open datum to the closed datum:
	\[
		\mathsf{ovhd}' = \mathsf{ovhd}
	\]
	where $\mathsf{ovhd}$ is the ADA in the head UTxO not belonging to any L2 UTxO (minimum-UTxO overhead), set at initialisation time and unchanged for the head's lifetime.}
\end{menumerate}
...
\begin{menumerate}
	...
	stay unchanged and the new state is governed again by $\nuHead$
	\[
		\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v,s,\eta,\contesters,\tfinal,\mathsf{ovhd}) \xrightarrow[\mathsf{ContestType}]{\stContest} (\stClosed,\cid,\hydraKeys,\Tcontest,v',s',\eta',\contesters',\tfinal',\mathsf{ovhd})}
	\]
	...
	\[
		\txMint = \varnothing
	\]
	\item \textcolor{red}{The ADA overhead $\mathsf{ovhd}$ is preserved in the output closed datum:
	\[
		\mathsf{ovhd}' = \mathsf{ovhd}
	\]}
\end{menumerate}
...
\begin{menumerate}
	...
	\[
		\val_{\mathsf{head}}^{\mathsf{in}} = \bigoplus_{i=1}^{m} \val(o_i) \oplus \val_{\mathsf{burned}} \oplus \mathsf{ovhd}
	\]
	where $\mathsf{ovhd}$ is the ADA overhead carried from the closed datum, and $\val_{\mathsf{burned}}$ is the value of all burned head tokens.}
\end{menumerate}
...
subsequent steps:}
\[
  \textcolor{red}{(\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta,\mathsf{ovhd})}
\]
\textcolor{red}{where $\tfinal$ is the contestation deadline (carried from $\stClosed$), $\eta$ is the
current accumulator commitment, and $\mathsf{ovhd}$ is the ADA overhead in the head UTxO not
...
\begin{menumerate}
  ...
  \[
	\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v,s,\eta,\contesters,\tfinal) \xrightarrow[m]{\stPartialFanout} (\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta',\mathsf{ovhd})}
  \]
  \textcolor{red}{For a $\stFanoutProgress$ input:}
  \[
	\textcolor{red}{(\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta,\mathsf{ovhd}) \xrightarrow[m]{\stPartialFanout} (\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta',\mathsf{ovhd})}
  \]
  ...
  \item \textcolor{red}{Parameters $\cid$, $\hydraKeys$, $\tfinal$, and $\mathsf{ovhd}$ are preserved in the output $\stFanoutProgress$ datum.}
...
\begin{menumerate}
  ...
  \[
	\textcolor{red}{(\stFanoutProgress,\cid,\hydraKeys,\tfinal,\eta,\mathsf{ovhd}) \xrightarrow[m,\pi]{\stFinalPartialFanout} \stFinal}
  \]
  ...
  \[
    \textcolor{red}{\val_{\mathsf{head}}^{\mathsf{in}} = \bigoplus_{i=1}^{m} \val(o_i) \oplus \val_{\mathsf{burned}} \oplus \mathsf{ovhd}}
  \]
\end{menumerate}

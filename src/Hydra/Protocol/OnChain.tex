...
on-chain: \mtxInit{}~\ref{sec:init-tx}, \mtxIncrement{}~\ref{sec:increment-tx}, \mtxDecrement{}~\ref{sec:decrement-tx}, \mtxClose{}~\ref{sec:close-tx}, \mtxContest{}~\ref{sec:contest-tx}, and \mtxFanout{}~\ref{sec:fanout-tx}. \\
...
However, it is \textbf{crucial} that all head members check that head output
always contains an $\st$ token of policy $\cid$ which satisfies
$\cid = \hash(\muHead(\seed))$. The $\seed$ from a head datum can be used to
determine this. Also, head members should verify whether the correct
verification key hashes are used in the $\pt$s and the open state is consistent
with parameters agreed during setup. See the initialTx behavior in

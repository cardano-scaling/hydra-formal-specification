...
creates BLS accumulator commitments to UTxO sets. When all UTxOs fit in a single transaction,
\mtxFanout{} distributes them all at once. When UTxO sets are too large, partial fanout transactions
use exclusion proofs to distribute subsets of UTxOs across multiple transactions while maintaining 
cryptographic guarantees about the remaining UTxOs.} \\
...
\begin{menumerate}
  ...
  \[
	\combine(\underline{C}) = \textcolor{red}{\accUTxO}(\{b \mid b \in \underline{C}\})
  \]
  \textcolor{red}{That is, given a list of committed UTxO $\underline{C}$, where each element is
  a bytestring corresponding to an output, $\combine$ creates a BLS accumulator commitment
  to the set of bytestrings that supports partial fanout.}
...
\textcolor{red}{\todo{Update closeTx.svg in Miro: output $\stClosed$ state should show single $\eta$ instead of three separate accumulators}}
...
\begin{menumerate}
  ...
  stay unchanged and the new state is governed again by $\nuHead$. \textcolor{red}{The output state
  contains a single unified accumulator $\eta'$ that combines all relevant UTxO sets
  (main state, increments, and decrements) using $\accCombine$:}
  \[
  ...
  \begin{menumerate}
	...
	\[
	  \eta' = \eta
	\]
    \item $\mathsf{Any}$: Closing snapshot refers to current state version $v$ \textcolor{red}{with no pending increments or decrements. The unified accumulator is simply the snapshotted state:}
	  \[
		\textcolor{red}{\eta' = \eta_{\text{snapshot}}}
	  \]
	  \[
		\textcolor{red}{\msVfy(\hydraKeys,(\cid || v || s' || \eta' || \bot || \bot),\xi) = \true}
	  \]
	  \item $\mathsf{UnusedInc}$: Closing snapshot refers to current state version $v$ \textcolor{red}{with pending increments that should be combined with the snapshotted state:}
	  \[
	    \eta_\alpha  \neq \bot
	  \]
	  \[
	    \textcolor{red}{\eta' = \accCombine(\eta_{\text{snapshot}}, \eta_\alpha)}
	  \]
	  \[
		\textcolor{red}{\msVfy(\hydraKeys,(\cid || v || s' || \eta_{\text{snapshot}} || \eta_\alpha || \bot),\xi) = \true}
	  \]
	  where $\eta_\alpha$ is provided by the redeemer \textcolor{red}{and combined with the snapshotted state to create the unified accumulator.}
	\item $\mathsf{UnusedDec}$: Closing snapshot refers to current state version $v$ \textcolor{red}{with pending decrements that should be combined with the snapshotted state:}
	\[
	  \textcolor{red}{\eta_\omega \neq \bot}
	\]
	\[
	  \textcolor{red}{\eta' = \accCombine(\eta_{\text{snapshot}}, \eta_\omega)}
	\]
	\[
	  \textcolor{red}{\msVfy(\hydraKeys,(\cid || v || s' || \eta_{\text{snapshot}} || \bot || \eta_\omega),\xi) = \true}
	\]
	\item $\mathsf{UsedInc}$: Closing snapshot refers to the previous state $v - 1$ \textcolor{red}{and includes increments that should be combined with the snapshotted state:}
	  \[
		\textcolor{red}{\eta_\alpha \neq \bot}
	  \]
	  \[
	    \textcolor{red}{\eta' = \accCombine(\eta_{\text{snapshot}}, \eta_\alpha)}
	  \]
	  \[
		\textcolor{red}{\msVfy(\hydraKeys,(\cid || v - 1 || s' || \eta_{\text{snapshot}} || \eta_\alpha || \bot ),\xi) = \true}
	\]
	\item $\mathsf{UsedDec}$: Closing snapshot refers to the previous state $v - 1$ \textcolor{red}{and reflects decrements. The unified accumulator is the snapshotted state:}
	  \[
	    \textcolor{red}{\eta_\omega \neq  \bot}
	  \]
	  \[ 
	  ...
	  \]
	\[
	  \textcolor{red}{\msVfy(\hydraKeys,(\cid || v - 1 || s' || \eta' || \bot || \eta_\omega),\xi) = \true}
	\]
	where $\eta_\omega$ is provided by the redeemer \textcolor{red}{for signature verification, but the decrements are already reflected in $\eta_{\text{snapshot}}$.}
  \end{menumerate}
...
\textcolor{red}{\todo{Update contestTx.svg in Miro: both input and output $\stClosed$ states should show single $\eta$ instead of three separate accumulators}}

\begin{figure}
  \includesvg[width=0.8\textwidth]{Hydra/Protocol/Figures/contestTx.svg}
  \caption{\mtxContest{} transaction spending the $\stClosed$ head output \textcolor{red}{with unified accumulator $\eta$} and
	producing a different $\stClosed$ head output \textcolor{red}{with updated $\eta$.}}\label{fig:contestTx}
\end{figure}
...
\begin{menumerate}
  \item \textcolor{red}{State remains in $\stClosed$ but with updated snapshot. Parameters $\cid,\hydraKeys,\Tcontest$
  stay unchanged and the new state is governed again by $\nuHead$. The output state
  contains a single unified accumulator $\eta'$ computed using $\accCombine$ based on the contest type:}
  \[
  ...
  \begin{menumerate}
	\item $\mathsf{Current}$: Contesting snapshot refers to current state version $v$ \textcolor{red}{with no pending increments or decrements. The unified accumulator is simply the snapshotted state:}
	  \[
		\textcolor{red}{\eta' = \eta_{\text{snapshot}}}
	  \]
	  \[
	...
	\item $\mathsf{UnusedInc}$: Contesting snapshot refers to current state version $v$ \textcolor{red}{with pending increments that should be combined with the snapshotted state:}
	  \[
	    \textcolor{red}{\eta_\alpha  \neq \bot}
	  \]
	  \[
	...
	\item $\mathsf{UnusedDec}$: Contesting snapshot refers to current state version $v$ \textcolor{red}{with pending decrements that should be combined with the snapshotted state:}
	\[
	  \textcolor{red}{\eta_\omega \neq \bot}
	\]
	\[
	...
	\[
	  \textcolor{red}{\msVfy(\hydraKeys,(\cid || v || s' || \eta_{\text{snapshot}} || \bot || \eta_\omega),\xi) = \true}
	\]
	  \item $\mathsf{UsedInc}$: Contesting snapshot refers to the previous state $v - 1$ \textcolor{red}{and includes increments that should be combined with the snapshotted state:}
	  \[
		\textcolor{red}{\eta_\alpha \neq \bot}
	  \]
	  \[
	...
	\item $\mathsf{UsedDec}$: Contesting snapshot refers to the previous state $v - 1$ \textcolor{red}{and reflects decrements. The unified accumulator is the snapshotted state:}
	  \[ 
	    \textcolor{red}{\eta_\omega \neq  \bot}
	  \]
	  \[ 
...
Once the contestation phase is over, a head may be finalized by posting a \mtxFanout{}
transaction (see Figure~\ref{fig:fanoutTx}), which distributes all UTxOs from the head
according to the \textcolor{red}{unified accumulator in the closed state. A fanout transaction consists of}
\begin{itemize}
  \item one input spending from $\nuHead$ holding the $\st$, and
  \item outputs $\textcolor{red}{o_{1} \dots o_{m}}$ to distribute all UTxOs.
\end{itemize}

\textcolor{red}{\todo{Update fanoutTx.svg in Miro: input $\stClosed$ state should show single $\eta$ instead of three separate accumulators}}
...
\begin{figure}
  ...
  \caption{\mtxFanout{} transaction spending the $\stClosed$ head output \textcolor{red}{with unified accumulator $\eta$} and
	distributing all funds with outputs $\textcolor{red}{o_{1} \dots o_{m}}$.}\label{fig:fanoutTx}
\end{figure}

\noindent The state-machine validator $\nuHead$ is spent with
\textcolor{red}{$\redeemerHead = (\mathsf{fanout}, m)$, where $m$ is the number of outputs
to distribute from the $\stClosed$ state.} The validator checks:
\begin{menumerate}
  \item State is advanced from $\datumHead \sim \stClosed$ to terminal state $\stFinal$:
  \[
	\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v, s,\eta,\contesters,\tfinal) \xrightarrow[m]{\stFanout} \stFinal}
  \]
  \item \textcolor{red}{All $m$ outputs are distributing the complete UTxO set according to the unified accumulator $\eta$. That is,
  the outputs exactly correspond to the UTxO set $U$ committed in the accumulator:}
  \[
	\textcolor{red}{\eta = \accUTxO(\{o_1, \ldots, o_m\})}
  \]
...
\subsubsection{Partial Fan-Out Transaction}\label{sec:partial-fanout-tx}

\textcolor{red}{When UTxO sets exceed transaction size limits or other constraints (see Section~\ref{sec:bls-accumulators}),
the protocol uses partial fanout to distribute a subset of UTxOs while proving that the remaining
UTxOs are still valid. A partial fanout transaction consists of}
\begin{itemize}
  \item \textcolor{red}{one input spending from $\nuHead$ holding the $\st$, and}
  \item \textcolor{red}{outputs $o_{1} \dots o_{m}$ to distribute a subset of UTxOs, and}
  \item \textcolor{red}{an output with updated $\stClosed$ state containing the remaining UTxOs.}
\end{itemize}

\textcolor{red}{\todo{Import partialFanoutTx.svg figure from Miro board}}

\noindent \textcolor{red}{The state-machine validator $\nuHead$ is spent with
$\redeemerHead = (\mathsf{partialFanout}, m, \pi, \eta')$, where:}
\begin{itemize}
  \item \textcolor{red}{$m$ is the number of outputs to distribute from the $\stClosed$ state}
  \item \textcolor{red}{$\pi$ is the exclusion proof for the distributed UTxO set}
  \item \textcolor{red}{$\eta'$ is the updated accumulator commitment for remaining UTxOs}
\end{itemize}
\textcolor{red}{The validator checks:}
\begin{menumerate}
  \item \textcolor{red}{State remains in $\stClosed$ with updated accumulator for remaining UTxOs:}
  \[
	\textcolor{red}{(\stClosed,\cid,\hydraKeys,\Tcontest,v, s,\eta,\contesters,\tfinal) \xrightarrow[m,\pi]{\stPartialFanout} (\stClosed',\cid,\hydraKeys,\Tcontest,v, s,\eta',\contesters,\tfinal)}
  \]
  \item \textcolor{red}{The $m$ outputs are distributing a subset of funds according to the unified accumulator $\eta$. That is,
  the exclusion proof $\pi$ verifies that the distributed outputs $S = \{o_1, \ldots, o_m\}$ 
  can be removed from the accumulator $\eta$:}
  \[
	\textcolor{red}{\accVerifyPartial(\eta, S, \pi) = \true}
  \]
  \item \textcolor{red}{The outgoing $\stClosed'$ datum contains the updated accumulator commitment 
  $\eta'$ that correctly represents the remaining UTxOs after removing the distributed set.}
  \item \textcolor{red}{Transaction is posted after contestation deadline $\txValidityMin > \tfinal$.}
  \item \textcolor{red}{All tokens are burnt
  $|\{\cid \mapsto \cdot \mapsto -1\} \in \txMint| = n + 1$.}
\end{menumerate}

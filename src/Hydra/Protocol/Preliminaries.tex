...
\subsection{\red{BLS Accumulator}}\label{sec:bls-accumulator}

{\color{red}
To enable efficient verification for large UTxO sets, the protocol uses a
cryptographic accumulator based on pairing-friendly elliptic curves and
KZG polynomial commitments~\cite{KZG10}. The accumulator commits to a combined
ordered sequence of outputs from the snapshot UTxO set and any pending
increment/decrement outputs, enabling succinct proofs for partial fan-out.

\noindent\textbf{UTxO hashing.}\quad For a UTxO set $U$, the canonical hash $\combine(U)$ is computed by
sorting outputs by their output references, serialising each output, concatenating the results,
and hashing:
\[
  \combine(U) = \hash\!\left(\bigoplus_{o \,\in\, \sortOn(\mathsf{outRef},\, U)} \bytes(o)\right)
\]
Canonical ordering is required so that all parties compute the same digest from the same UTxO set.
The plain $\hash$ function denotes a generic collision-resistant hash applied to a bytestring.

\noindent\textbf{Accumulator operations.}\quad The following operations are used:
\begin{itemize}
  \item $A \gets \accUTxO(U)$ computes an accumulator commitment (a BLS12-381 G1 element) to
        a UTxO set $U$. When committing to a snapshot with pending increments $U_\alpha$ and
        decrements $U_\omega$, the combined set $U \cup U_\alpha \cup U_\omega$ is passed.
  \item $A_\varnothing$ denotes the \emph{empty accumulator}: the BLS12-381 G1 generator,
        representing a commitment to the empty set.
  \item $\updateVfy(A, A', S, \mathit{crs}) \in \tyBool$ verifies that $A'$ is the
        accumulator obtained from $A$ by removing the set of outputs $S$, using the
        common reference string $\mathit{crs}$. Concretely, this is a KZG pairing check
        $e(A, G_2) = e(A', P_S(\tau) \cdot G_2)$, where $P_S(X) = \prod_{s_i \in S}(X - s_i)$
        and the $s_i$ are integer encodings of the output hashes.
\end{itemize}

\noindent\red{\textbf{Accumulator commitments in signed messages.}\quad When an accumulator commitment
$A$ appears as a component of a multi-signature message, it is first serialised to a
canonical bytestring $\bytes(A)$ before concatenation with the other message components.
The serialisation must be deterministic so that all parties produce the same bytestring
from the same commitment.}

\noindent A trusted setup ceremony provides the common reference string $\mathit{crs}$.
The protocol uses the EIP-4844 KZG trusted setup as $\mathit{crs}$, delivered
on-chain via a dedicated reference script output ($\mathtt{vCRS}$) to avoid
re-embedding the large G2 points in every transaction.
}

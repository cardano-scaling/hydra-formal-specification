...
\noindent\textbf{UTxO hashing.}\quad For a UTxO set $U$, the hash $\hash(U)$ is computed by
sorting outputs canonically by their output references, serialising each output, concatenating
the results, and hashing:
\[
  \hash(U) = \hash\!\left(\bigoplus_{o \,\in\, \sortOn(\mathsf{outRef},\, U)} \bytes(o)\right)
\]
Canonical ordering is required so that all parties compute the same hash from the same UTxO set.

...
\noindent \textcolor{red}{The off-chain protocol coordinates BLS accumulator operations (see Section~\ref{sec:bls-accumulator})
for partial fanout support. When UTxO sets are too large for single transactions (see Section~\ref{sec:bls-accumulator}
...
The protocol is specified as a
...
\begin{enumerate}
	\item On-chain protocol transactions as introduced in
	      ...
	      \begin{itemize}
		      ...
		      \item \textcolor{red}{$\mathtt{partialFanoutTx}$: distributes a subset of UTxOs from a closed head (intermediate)}
		      \item $\mathtt{fanoutTx}$: distributes all UTxOs from a closed head (final, no pending partial fanout)
		      \item \textcolor{red}{$\mathtt{finalPartialFanoutTx}$: distributes the last subset and finalizes a partially fanned-out head}
	      \end{itemize}
...
\begin{itemize}
	...
	\item $\hatmU$: UTxO set of the latest seen (in-flight) snapshot, used when collecting acknowledgements in $\hpAS$.
	...
	\item $\bar{\mc S}$: Snapshot object of the latest confirmed snapshot which
	      ...
	      \begin{center}
		      \begin{tabular}{|l|l|}\hline
			      ...
			      $\bar{\mc S}.A$         & \red{BLS accumulator commitment $A = \accUTxO(U \cup U_\alpha \cup U_\omega)$} \\ \hline
			      $\bar{\mc S}.\sigma$   & multisignature                                              \\ \hline
		      \end{tabular}
	      \end{center}
\end{itemize}

where constructor $\text{snObj}(v, n, T, U, U_\alpha, U_\omega, A)$ initializes a
...
$\bar{\mc S} \gets \Sno(0, 0, [], \emptyset, \emptyset, \emptyset, \red{\accUTxO(\emptyset)})$.
...
$\hats$, the UTxO hashes $\eta = \hash(U)$,
$\eta_\alpha = \hash(U_\alpha)$, $\eta_\omega = \hash(\mathsf{outputs}(\tx_\omega))$,
and the \red{BLS accumulator commitment $A = \accUTxO(U \cup U_\alpha \cup \mathsf{outputs}(\tx_\omega))$}
(see Section~\ref{sec:bls-accumulator}). The signature is sent to all head members via
...
$\bar{\mc S} \gets \Sno(\hatv, \hats, \hatmT, \hatmU, U_{\alpha}, \mathsf{outputs}(\tx_{\omega}), \red{\accUTxO(\hatmU \cup U_{\alpha} \cup \mathsf{outputs}(\tx_{\omega}))})$
...
transaction providing the confirmed snapshot accumulator commitments. If, however,
there was a pending deposit, any participant can now submit an $\mathtt{incrementTx}$
by providing the confirmed snapshot accumulator commitments. Lastly, if $\party_i$ is the next snapshot
...
construct the $\mtxClose$ transaction using the accumulator commitments and
the certificate $\xi$ from the corresponding
multi-signature. See Section~\ref{sec:close-tx} for details about this
transaction. \\

\dparagraph{$\mathtt{closeTx}/\mathtt{contestTx}$.}\quad When a party observes
the head getting closed or contested, the on-chain snapshot represents the latest head status
aggregated so far (by a sequence of \mtxClose{} and
\mtxContest{} transactions). If the last confirmed (off-chain) snapshot is newer
than the observed (on-chain) snapshot number $s_{c}$, a \mtxContest{} transaction
is posted with the certificate $\xi$ and accumulator commitments (see Section~\ref{sec:contest-tx}). \\

\subsubsection{Finalizing the head}

\dparagraph{$\mathtt{fanoutTx}$.}\quad Once the contestation deadline $\tfinal$
has passed, any party may finalize the head by posting a $\mtxFanout{}$ transaction.
This is possible when the confirmed snapshot UTxO set $\bar{\mc S}.U$ is small enough to
distribute in a single transaction. The head transitions to $\stFinal$, burning all head
tokens and paying all UTxOs to their respective owners. \\

\dparagraph{\textcolor{red}{$\mathtt{partialFanoutTx}$.}}\quad \red{When the
confirmed snapshot UTxO set $\bar{\mc S}.U$ is too large to distribute in a single
transaction, a party instead posts a sequence of $\mtxPartialFanout{}$ transactions,
each distributing the next chunk of UTxOs. Each intermediate step stores the updated
accumulator commitment $A'$ in the output datum, verified on-chain using
$\updateVfy(A, A', S, \mathit{crs})$. After all but the last chunk are distributed,
a $\mathtt{finalPartialFanoutTx}$ transaction distributes the last chunk with the
empty accumulator $A_\varnothing$ as the expected residual (see
Section~\ref{sec:final-partial-fanout-tx}), burns all head tokens, and transitions
the head to $\stFinal$. Off-chain coordination of partial fanout is handled similarly
to the nominal fanout case.}

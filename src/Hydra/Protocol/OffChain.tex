...
\begin{enumerate}
	\item On-chain protocol transactions as introduced in
	      ...
	      \begin{itemize}
		      ...
		      \item \textcolor{red}{$\mathtt{fanoutTx}$: distributes all UTxOs from a closed head in one step}
		      \item \textcolor{red}{$\mathtt{partialFanoutTx}$: distributes a subset of UTxOs, transitioning to $\stFanoutProgress$ (intermediate)}
		      \item \textcolor{red}{$\mathtt{finalPartialFanoutTx}$: distributes the last subset and burns tokens (terminal from $\stFanoutProgress$)}
	      \end{itemize}
...
\textcolor{red}{\dparagraph{$\mathtt{partialFanoutTx}$.}\quad Upon observing a $\mtxPartialFanout{}$
transaction, the distributed UTxOs are removed from the local state tracking and the head transitions
to the $\stFanoutProgress$ state with an updated accumulator commitment for the remaining UTxOs.
Upon observing a $\mtxFinalPartialFanout{}$ transaction, the remaining UTxOs are distributed and
the head transitions to $\stFinal$ state.
\todo{Add detailed off-chain protocol for partial fanout coordination, including
selection strategy for UTxO subsets and when to use $\mtxPartialFanout{}$ versus $\mtxFinalPartialFanout{}$.}}

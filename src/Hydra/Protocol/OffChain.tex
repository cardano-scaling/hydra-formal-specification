...
$\hats$, and the \red{BLS accumulator commitments} $\eta = \red{\accUTxO}(U)$,
$\eta_\alpha = \red{\accUTxO}(U_\alpha)$, $\eta_\omega = \red{\accUTxO}(\mathsf{outputs}(\tx_\omega))$
...
$\bar{\mc S} \gets \Sno(\hatv, \hats, \hatmT, \hatmU, U_{\alpha}, \mathsf{outputs}(\tx_{\omega}), \accUTxO(\hatmU))$
...
each distributing the next chunk of UTxOs using the BLS accumulator commitment stored
in the closed datum, with an exclusion proof generated using
$\membershipProof(S, A, \mathit{crs})$ for each chunk $S$. The head remains in
$\stClosed$ with an updated accumulator reflecting the remaining UTxOs after each
partial fanout. After all UTxOs are distributed, a $\mathtt{finalPartialFanoutTx}$
transaction distributes the last chunk and burns all head tokens, transitioning the
head to $\stFinal$. Off-chain coordination of partial fanout is handled similarly

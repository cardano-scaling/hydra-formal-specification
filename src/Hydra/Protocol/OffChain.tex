...
\begin{enumerate}
  \item On-chain protocol transactions as introduced in
        ...
        \begin{itemize}
          ...
          \item \textcolor{red}{$\mathtt{fanoutTx}$: distributes all UTxOs from a closed head (final)}
...
\noindent\textcolor{red}{\dparagraph{$\mathtt{fanoutTx}$.}\quad Upon observing a \mtxFanout{}
transaction, all UTxOs are distributed and the head transitions to $\stFinal$ state.}

\noindent\textcolor{red}{\dparagraph{$\mathtt{partialFanoutTx}$.}\quad Upon observing a partial fanout
transaction, the distributed UTxOs are removed from the local state tracking and the head remains
in $\stClosed$ state with updated accumulator containing the remaining UTxOs.
The off-chain protocol must coordinate partial fanout to

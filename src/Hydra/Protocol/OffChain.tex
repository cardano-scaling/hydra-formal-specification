...
for partial fanout support. When UTxO sets are too large for a single fanout transaction, the
off-chain protocol automatically falls back to partial fanout, posting a sequence of
$\mtxPartialFanout{}$ transactions followed by a $\mtxFinalPartialFanout{}$ transaction,
ensuring all UTxOs can be distributed across multiple transactions while maintaining cryptographic integrity.}
...
$\hats$, the UTxO hashes $\eta = \combine(U)$,
$\eta_\alpha = \combine(U_\alpha)$, $\eta_\omega = \combine(\mathsf{outputs}(\tx_\omega))$,
...
either increment or decrement UTxO set ($\eta_\alpha$ or $\eta_\omega$), \red{the
accumulator commitment $\bar{\mc S}.A$,} and the certificate
...
\red{the accumulator commitment $\bar{\mc S}.A$,} and certificate $\xi$ is constructed
and posted in a \mtxContest{} transaction (see Section~\ref{sec:contest-tx}).

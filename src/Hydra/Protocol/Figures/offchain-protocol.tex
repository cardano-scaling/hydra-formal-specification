			...
			\begin{tabular}{c}
				...
				\adjustbox{valign=t,scale=\sfact}{
					\begin{walgo}{0.7}
						...
						$\hatmL \gets \emptyset$ \;
						$\bar{\mc S} \gets \Sno(0, 0, [], \emptyset, \emptyset, \emptyset, \accUTxO(\emptyset))$ \;
						$\hatv, \hats \gets 0$ \;
						$\hatmT \gets \emptyset$ \;
						$\tx_\omega \gets \bot$ \;
						$\tx_\alpha \gets \bot$ \;
					...
					\begin{walgo}{0.65}
						...
						\On{$(\hpRS,v,s,\underline{\tx}_{\mathsf{req}} ,\tx_\alpha,\tx_\omega)$ from $\party_j$}{
							...
							\Wait{$\hats = \bar{\mc S}.s ~ \land ~ v = \hatv$}{
								...
								$\eta \gets \hash(U)$ \;
								$\eta_\alpha \gets \hash(U_\alpha)$ \;
								$\eta_\omega \gets \hash(\mathsf{outputs}(\tx_\omega))$ \;
								$A \gets \accUTxO(U \cup U_\alpha \cup \mathsf{outputs}(\tx_\omega))$ \;
								$\msSig_i \gets \msSign(\hydraSigningKey, (\cid || v || \hats || \eta || \eta_\alpha || \eta_\omega || A))$ \;
						...
						\On{$(\hpAS,s,\msSig_j)$ from $\party_j$}{
							...
							\Wait{$\hats=s$}{
								...
								\If{$\forall k \in [1..n]: (k,\cdot) \in \hatSigma$}{
									...
									$\eta \gets \hash(\hatmU)$ \;
									$\eta_\alpha \gets \hash(U_\alpha)$ \;
									$\eta_\omega \gets \hash(U_\omega)$ \;
									$A \gets \accUTxO(\hatmU \cup U_\alpha \cup U_\omega)$ \;
									% NOTE: Implementation differs here and
									% below as it stores seen version in seen
									% snapshot and uses that to verify
									\Req{} $\msVfy(\hydraKeysAgg, (\cid || \hatv || \hats || \eta || \eta_\alpha || \eta_\omega || A), \msCSig)$ \;
									% create confirmed snapshot for later reference
									$\bar{\mc S} \gets \Sno(\hatv, \hats, \hatmT, \hatmU, U_\alpha, U_\omega, A)$ \;
			...
			\adjustbox{valign=t,scale=\sfact}{
				\begin{walgo}{0.6}
					...
					\On{$(\hpClose)$ from client}{
						$\eta \gets \hash(\bar{\mc S}.U)$ \;
						$\eta_\alpha \gets \hash(\bar{\mc S}.U_\alpha)$ \;
						$\eta_\omega \gets \hash(\bar{\mc S}.U_\omega)$ \;
						$\xi \gets \bar{\mc S}.\sigma$ \;
						% XXX: \hatv needed to distinguish between CloseType redeemer, explain how exactly?
						$\PostTx{}~(\mtxClose, \hatv, \bar{\mc S}.v, \bar{\mc S}.s, \eta, \eta_\alpha, \eta_\omega, \bar{\mc S}.A, \xi)$ \;
					}
				\end{walgo}
			}
			\adjustbox{valign=t,scale=\sfact}{
				\begin{walgo}{0.6}
					...
					\On{$(\gcChainClose, \eta) \lor (\gcChainContest, s_{c}, \eta)$ from chain}{
						\If{$\bar{\mc S}.s > s_{c}$}{
							...
							$\xi \gets \bar{\mc S}.\sigma$ \;
							% XXX: \hatv needed to distinguish between CloseType redeemer, explain how exactly?
							$\PostTx{}~(\mtxContest, \hatv, \bar{\mc S}.v, \bar{\mc S}.s, \eta, \eta_\alpha, \eta_\omega, \bar{\mc S}.A, \xi)$ \;
						}
					}
				\end{walgo}
			}

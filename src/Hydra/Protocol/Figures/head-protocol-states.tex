\begin{figure}[t!]
  ...
  \begin{tikzpicture}[>=stealth,auto,node distance=2.8cm, initial text=$\mathsf{init}$, every
    ...
    \path[->] (closed) edge [loop above] node {$\stContest$} (closed);
      \textcolor{red}{\path[->] (fprogress) edge [bend right=20] node [sloped] {$\stFanout$} (final);}
      \textcolor{red}{\path[->] (closed) edge [bend right=20] node [sloped] {$\stFinalPartialFanout$} (fprogress);}

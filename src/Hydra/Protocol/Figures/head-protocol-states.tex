\begin{figure}[t!]
  ...
  \newline
  \newline
  \begin{tikzpicture}[>=stealth,auto,node distance=3cm, initial text=$\mathsf{init}$, every
    state/.style={text width=12mm, align=center}]
    \node[state, initial] (open) {$\stOpen$};
    \node[state] (closed) [right of=open] {$\stClosed$};
    \node[state, accepting] (final) [right of=closed] {$\stFinal$};
    \node[state, text width=18mm] (fanoutProgress) at (4.5,-2.8) {$\stFanoutProgress$};
    ...
    \path[->] (closed) edge node [left] {$\stPartialFanout$} (fanoutProgress);
    \path[->] (fanoutProgress) edge [loop below] node {$\stPartialFanout$} (fanoutProgress);
    \path[->] (fanoutProgress) edge node [right] {$\stFinalPartialFanout$} (final);
  \end{tikzpicture}

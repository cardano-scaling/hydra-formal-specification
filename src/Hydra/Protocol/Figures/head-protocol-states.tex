\begin{figure}[t!]
  ...
  \begin{tikzpicture}[>=stealth,auto,node distance=2.8cm, initial text=$\mathsf{init}$, every
    ...
    \textcolor{red}{\node[state] (fprogress) [below of=closed] {$\stFanoutProgress$};}
    ...
    \textcolor{red}{\path[->] (closed) edge node {$\stPartialFanout$} (fprogress);}
    \textcolor{red}{\path[->] (fprogress) edge [loop below] node {$\stPartialFanout$} (fprogress);}
    \textcolor{red}{\path[->] (fprogress) edge node {$\stFinalPartialFanout$} (final);}
  \end{tikzpicture}
